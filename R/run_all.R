...
# Single-file pipeline: clean, join, analyze, visualize.
#
...
setwd("/home/harsh/Desktop/stat184-demos/Technocrats")
...
merged <- youth_unemp |>
  ...
  mutate(
    ...
    income_group = countrycode(iso3c, "iso3c", "wb",        warn = FALSE)
  )

saveRDS(merged, "data/merged.rds")
write_csv(merged, "data/merged.csv")

cat(sprintf("Merged panel: %d rows | %d countries | years %d-%d\n",
...
# ---- 2a. Scatter plots colored by continent ---------------------------------
...
plots_scatter <- imap(outcome_labels, \(label, name) scatter_plot(name, label))
...
p_corr <- ggcorrplot(
  ...
  title  = "Correlation: Internet & Generational Outcomes",
...
p_facet <- merged |>
  ...
  labs(
    ...
    x = "Internet Users (% of population)", y = NULL,
    caption = "Sources: World Bank WDI, IHME GBD 2023, OECD"
  ) +
...
# ---- 2d. Linear regression coefficient plot --------------------------------

run_reg <- function(outcome) {
  df <- merged |> filter(!is.na(internet), !is.na(.data[[outcome]]))
  lm(reformulate("internet", outcome), data = df)
}

models  <- map(names(outcome_labels), run_reg) |> setNames(names(outcome_labels))
...
p_coef <- coef_df |>
  ...
  labs(
    title = "OLS: Effect of Internet Penetration on Each Outcome",
    x = "Coefficient (per 1 pp increase in internet users)",
    y = NULL, color = NULL,
    caption = "95% CIs. Sources: World Bank WDI, IHME GBD 2023, OECD"
  ) +
  theme_minimal(base_size = 13) +
  theme(legend.position = "bottom")
